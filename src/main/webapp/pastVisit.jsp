            ...
            <div style="display: flex; flex-direction: column; gap: 6px; padding: 20px; border: 2px solid green; border-radius: 16px; justify-content: center; align-items: center; padding-top: 12px">
                <h3>Past appointment</h3>
                    <div style="display: flex; flex-direction: column; gap: 16px">
                        <span>Appointment date: ${visit.appointmentDateTime}</span>
                        <span>Dentist: ${visit.dentist}</span>
                        <span>
                            Procedures:
                            [<c:forEach items="${visit.procedures}" var="proced">
                                ${proced.procedureName}
                            </c:forEach>]
                        </span>
                        <label for="medicalDescription">Medical description</label>
                        <textarea id="medicalDescription">${visit.medicalDescription}</textarea>
                    </div>
                <c:if test="${not empty error}">
                    <div style="color: red">${error}</div>
                </c:if>
                <a class="back" href="/pastVisits">Back</a>
            </div>
        </div>
    </div>
</div>
